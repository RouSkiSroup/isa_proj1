...
long getLong(string str) {
    for (long i = 0; i < str.length(); i++) {
        if (!(str[i] >= '0' && str[i] <= '9')) {
            cout << "ERROR: Failed to convert " << str << " to number " << endl;
            exit(-1);
        }
        if (str.length() == 0) {
            cout << "ERROR: Failed to convert " << str << " to number" << endl;
    ...
    if (res > LONG_MAX) { //range for port.
        cout << "ERROR: Out of range. Max " << LONG_MAX << endl;
        exit(-1);
    }
    return res;
}
...
void loadArgs(int argc, char *argv[], myArgs *args) {
	...
	if (!((args->host_set) && (args->port_set))) {
		cerr << "ERROR:  Options -H and -p are required." << endl;
		exit(-1);
	}
