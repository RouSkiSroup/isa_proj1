...
\
//function to close socket before ctrl+c exit
...
//            cout << "Failed to convert " << str << " to number" << endl;
    ...
    if (res > LONG_MAX) { //max port range.
...
//str - string number
//if fails, returns 65535, which will thow response 404 Not found.
long getLongPort(string str) {
    for (long i = 0; i < str.length(); i++) {
        if (!(str[i] >= '0' && str[i] <= '9')) {
            cerr << "ERROR: Failed to convert " << str << " to number" << endl;
            return -1;
        }
        if (str.length() == 0) {
            cerr << "ERROR: Failed to convert " << str << " to number" << endl;
    ...
    res = strtol(str.c_str(), &end, 10);
    if (res > LONG_MAX) { //max port range.
        cerr << "ERROR: Out of range. Max " << LONG_MAX << endl;
        return -1;
    }
    return res;
}
...
string cmdBoardDelete(commandDataStruct command_data, vector<boardStruct> *boards_list){
    ...
    if(exists == -1){
        ...
        (*boards_list).erase((*boards_list).begin()+exists);
        header=string("HTTP/1.1 200 OK \r\n\r\n");
    };
    return header;
}
...
void checkArgCount(int argc){
    if (argc != 3){
        cerr << "ERROR: Bad argument count!" << endl;
        exit(-1);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (strcmp(argv[1],"-p") != 0){
        cerr << "ERROR: The program accepts only parameter -p!" << endl;
    }
    long port = getLongPort(argv[2]);
    if (port == -1){
...
//    printBoardsList(boards_list);
    ...
    int server_fd;
//    int new_socket; declared as global
    long valread;
